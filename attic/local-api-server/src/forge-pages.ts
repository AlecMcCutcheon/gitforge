...
import { parseWhoami } from "./forge-registry.js";
import { runCommand } from "./run.js";
import { ensureTipPack } from "./tip-browse.js";

export type ForgePagesStatus = "off" | "ready" | "publishing" | "error";
...
interface PagesFile {
  schema_version: number;
  repos: Record<string, ForgePagesConfig>;
}

const syncLocks = new Map<string, Promise<ForgePagesConfig>>();

function pagesBaseUrl(): string {
  return (
    process.env.GITFORGE_PAGES_BASE?.replace(/\/$/, "") ??
...
async function pagesPath(): Promise<string> {
  ...
  return path.join(root, "forge-pages.json");
}
...
export async function getPages(prefix: string): Promise<ForgePagesConfig | null> {
...
export async function extractTipSiteDir(
  ...
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "freenet-forge-pages-"));
...
<body>
  ...
  <p>GitForge Pages for <strong>${label}</strong> is turned off. The last live site may remain on the network until replaced.</p>
</body>
</html>
`,
    "utf8",
...
export async function syncPages(prefix: string): Promise<ForgePagesConfig> {
...
export async function disablePages(
  ...
  if (options.tombstone && row.websiteKeyName) {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "freenet-forge-pages-off-"));
    try {
