...
export interface ForgeRegistration {
  ...
  /** Bridge attestation until Freenet ForgeRegistry WASM dual-sig is published. */
...
async function registryPath(): Promise<string> {
  ...
  return path.join(root, "forge-registry.json");
}
...
export async function listRegistry(): Promise<ForgeRegistration[]> {
...
export async function registerRepo(input: {
  ...
  const next: ForgeRegistration = {
    ...
    identity_name: parsed.name, // bridge legacy; website dual-sig writes empty + ForgeProfile for display
...
/** Soft-unregister from local bridge ForgeRegistry file. */
