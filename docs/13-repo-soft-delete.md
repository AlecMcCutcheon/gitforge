# Soft-delete repositories (Freenet / GitForge)
...
| Root file **`DELETED`** | Tip tree | Human-visible in clones and the GitForge file browser |
| Description prefix **`[deleted]`** | `RepoState.description` | Fallback for older UIs / tools that only show metadata |

**Detection order** (GitForge and tools):
...
4. ForgeRegistry **remove** (dual-signed) so Discover / People stop listing the repo.
5. After tombstone + unregister succeed: forge-identity **RemoveRepoKey** for that prefix.
6. If ForgeVault was already **in_sync** with the delegate, auto-push the reduced key set; otherwise leave vault alone (Settings → Vault & sync).
...
- Tip `DELETED` file: commit and `git push` after, or let GitForge rely on extension + description.

## ForgeRegistry

See `contracts/forge-registry/SCHEMA.md` — `{ "remove": … }` with domain
`gitforge.unregister.v1`. Tombstones keep peer merges from resurrecting a
listing until a higher-`seq` upsert re-registers.

## Related

- `docs/06-forge-registry.md`
