#!/usr/bin/env bash
# Build + package ForgeRegistry + ForgeVault + ForgeStars + ForgeProfile + forge-identity;
...
echo "==> Package ForgeRegistry contract (fdev)"
(cd contracts/forge-registry && fdev build --package-type contract)

echo "==> Package ForgeVault contract (fdev)"
(cd contracts/forge-vault && fdev build --package-type contract)

echo "==> Package ForgeStars contract (fdev)"
(cd contracts/forge-stars && fdev build --package-type contract)

echo "==> Package ForgeProfile contract (fdev)"
(cd contracts/forge-profile && fdev build --package-type contract)

echo "==> Package ForgeRepoMeta contract (fdev)"
(cd contracts/forge-repo && fdev build --package-type contract)

echo "==> Package forge-identity delegate (fdev)"
(cd delegates/forge-identity && fdev build --package-type delegate)

echo "==> Package forge-pages delegate (fdev)"
(cd delegates/forge-pages && fdev build --package-type delegate)

REGISTRY_PKG="$ROOT/contracts/forge-registry/build/freenet/freenet_forge_registry"
VAULT_PKG="$ROOT/contracts/forge-vault/build/freenet/freenet_forge_vault"
STARS_PKG="$ROOT/contracts/forge-stars/build/freenet/freenet_forge_stars"
PROFILE_PKG="$ROOT/contracts/forge-profile/build/freenet/freenet_forge_profile"
REPO_PKG="$ROOT/contracts/forge-repo/build/freenet/freenet_forge_repo"
IDENTITY_PKG="$ROOT/delegates/forge-identity/build/freenet/freenet_forge_identity"
PAGES_PKG="$ROOT/delegates/forge-pages/build/freenet/freenet_forge_pages"
REGISTRY_RAW="$ROOT/target/wasm32-unknown-unknown/release/freenet_forge_registry.wasm"
VAULT_RAW="$ROOT/target/wasm32-unknown-unknown/release/freenet_forge_vault.wasm"
STARS_RAW="$ROOT/target/wasm32-unknown-unknown/release/freenet_forge_stars.wasm"
PROFILE_RAW="$ROOT/target/wasm32-unknown-unknown/release/freenet_forge_profile.wasm"
REPO_RAW="$ROOT/target/wasm32-unknown-unknown/release/freenet_forge_repo.wasm"
IDENTITY_RAW="$ROOT/target/wasm32-unknown-unknown/release/freenet_forge_identity.wasm"
PAGES_RAW="$ROOT/target/wasm32-unknown-unknown/release/freenet_forge_pages.wasm"
...
node "$ROOT/scripts/gen-forge-owner-constants.mjs" \
...
CONTRACT_ID="$(fdev get-contract-id --code "$REGISTRY_RAW" --parameters <(printf 'gitforge-registry-v1') 2>/dev/null || true)"
printf 'gitforge-registry-v1' > "$ROOT/build/forge-registry-params.bin"
printf '{"schema_version":1,"repos":{}}' > "$ROOT/build/forge-registry-empty.json"
printf 'gitforge-stars-v1' > "$ROOT/build/forge-stars-params.bin"
printf '{"schema_version":1,"by_repo":{}}' > "$ROOT/build/forge-stars-empty.json"
...
| Artifact | Value |
|----------|-------|
| ForgeRegistry WASM BLAKE3 (base58) | \\\`\${regB58}\\\` |
| ForgeRegistry params (UTF-8) | \\\`gitforge-registry-v1\\\` |
| ForgeRegistry contract id | \\\`\${contractId}\\\` |
| ForgeVault WASM BLAKE3 (base58) | \\\`\${vaultB58}\\\` |
| ForgeVault params | \\\`gitforge-vault-v1:<vault_id_hex>\\\` |
| ForgeStars WASM BLAKE3 (base58) | \\\`\${starsB58}\\\` |
| ForgeStars params (UTF-8) | \\\`gitforge-stars-v1\\\` |
| ForgeProfile WASM BLAKE3 (base58) | \\\`\${profileB58}\\\` |
| ForgeProfile params | \\\`gitforge-profile-v1:<fingerprint>\\\` |
| ForgeRepoMeta WASM BLAKE3 (base58) | \\\`\${repoB58}\\\` |
| ForgeRepoMeta params | \\\`gitforge-repo-v1:<repo_prefix>\\\` |
