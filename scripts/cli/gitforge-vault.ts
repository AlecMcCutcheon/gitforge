#!/usr/bin/env npx tsx
/**
 * GitForge vault CLI — push/pull freenet-git bundles ↔ vault (schema v4).
 *
...
function usage(): never {
  console.error(`Usage:
  ${brand.cliName} vault sync-bundle --api-key <key> --bundle <path> [options]
  ${brand.cliName} vault pull-bundle  --api-key <key> --bundle <path> [options]
...
Shared options:
  ...
  --import-local-delegate    Also ImportRepoKey into this node's identity delegate
...
async function unlockReposFromVault(opts: {
  ...
  const state = await fetchForgeVault(opts.vault_id);
  if (!state?.identity_dek_wrap?.blob_b64) {
    throw new Error(
      `no ${brand.displayName} vault for this identity — create/restore identity in ${brand.displayName} first`,
    );
  }
  ...
  // );
  // NEW CODE - TESTING: normalize all envelope scope aliases
  const scopes = normalizeScopes(wrap.scopes);
  if (!scopes.includes(ENVELOPE_REPOS)) {
    throw new Error(
      `API key missing repos scope (have: ${wrap.scopes.join(", ") || "none"})`,
    );
  }
...
async function syncBundle(opts: {
  ...
  console.error(`${brand.displayName} vault repos envelope updated (ops-signed).`);

  if (opts.importLocalDelegate) {
    console.error("Importing repo keys into local forge-identity delegate…");
    let n = 0;
    for (const repo of opened.repos) {
...
async function pullBundle(opts: {
  ...
  if (opts.importLocalDelegate) {
    console.error("Importing vault repo keys into local forge-identity…");
    let n = 0;
...
/** Programmatic entry (unified `gitforge vault …`). */
...
const isDirectEntry =
  ...
  /gitforge-vault(\.ts)?$/.test(process.argv[1].replace(/\\/g, "/"));
