# HubStars contract (schema)

Public star index for FreenetHub repos. Anyone may read; only the starring
identity may add or remove its own entry under a `repo_prefix`.

## Parameters

UTF-8 `gitatlas-stars-v1` (singleton for a given WASM code hash).
...
**Star:** domain `gitatlas.star.v1\0` + nul fields `repo_prefix`, `fingerprint`,
`label` (empty if absent), `starred_at`.

**Unstar:** domain `gitatlas.unstar.v1\0` + nul fields `repo_prefix`,
