//! HubStars Freenet contract — per-repo star maps signed by identity.
...
fn apply_star(state: &mut HubStarsState, op: StarOp) -> Result<(), ContractError> {
...
fn apply_unstar(state: &mut HubStarsState, op: UnstarOp) -> Result<(), ContractError> {
    check_prefix(&op.repo_prefix).map_err(ContractError::Other)?;
    let vk = identity_vk_from_fingerprint(&op.fingerprint).map_err(ContractError::Other)?;
...
fn parse_state(bytes: &[u8]) -> Result<HubStarsState, ContractError> {
    if bytes.is_empty() {
        return Ok(HubStarsState::empty());
    }
    serde_json::from_slice(bytes).map_err(|e| ContractError::Deser(e.to_string()))
}

fn apply_bytes(state: &mut HubStarsState, bytes: &[u8]) -> Result<(), ContractError> {
    if bytes.is_empty() {
        return Ok(());
    }
    ...
    if let Ok(incoming) = serde_json::from_slice::<HubStarsState>(bytes) {
...
mod tests {
    ...
    fn star_and_unstar() {
        ...
        let mut state = HubStarsState::empty();
