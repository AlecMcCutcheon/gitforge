//! ForgeRegistry Freenet contract — public Discover/People listings.
//!
...
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq)]
pub struct ForgeRegistryRemove {
    pub schema_version: u32,
    pub repo_prefix: String,
    pub identity_fingerprint: String,
    pub repo_owner_vk: String,
...
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq)]
pub struct ForgeRegistryContributor {
    pub schema_version: u32,
    pub repo_prefix: String,
...
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq)]
pub struct ForgeRegistryPendingInvite {
    pub schema_version: u32,
    pub repo_prefix: String,
...
pub struct ForgeRegistryState {
    pub schema_version: u32,
    pub repos: BTreeMap<String, ForgeRegistryEntry>,
    #[serde(default)]
    pub removed: BTreeMap<String, ForgeRegistryRemove>,
    /// Accepted collaborators per repo_prefix → fingerprint.
    #[serde(default)]
    pub contributors: BTreeMap<String, BTreeMap<String, ForgeRegistryContributor>>,
    /// Outstanding invites per repo_prefix → invitee fingerprint.
    #[serde(default)]
    pub pending_invites: BTreeMap<String, BTreeMap<String, ForgeRegistryPendingInvite>>,
}

impl ForgeRegistryState {
    fn empty() -> Self {
        Self {
            schema_version: 1,
            repos: BTreeMap::new(),
...
pub fn signing_payload(entry: &ForgeRegistryEntry) -> Vec<u8> {
...
pub fn unregister_signing_payload(op: &ForgeRegistryRemove) -> Vec<u8> {
    let mut out = Vec::with_capacity(256);
...
pub fn contributor_add_signing_payload(op: &ForgeRegistryContributor) -> Vec<u8> {
    let mut out = Vec::with_capacity(256);
...
pub fn contributor_remove_signing_payload(op: &ForgeRegistryContributor) -> Vec<u8> {
    let mut out = Vec::with_capacity(256);
...
pub fn pending_invite_add_signing_payload(op: &ForgeRegistryPendingInvite) -> Vec<u8> {
    let mut out = Vec::with_capacity(256);
...
pub fn pending_invite_remove_signing_payload(op: &ForgeRegistryPendingInvite) -> Vec<u8> {
    let mut out = Vec::with_capacity(256);
...
fn validate_entry(entry: &ForgeRegistryEntry) -> Result<(), String> {
    if entry.schema_version != 1 {
        return Err(format!("unsupported schema_version {}", entry.schema_version));
    }
    if entry.attestation != "dual-sig-v1" {
        return Err(format!(
            "ForgeRegistry WASM only accepts dual-sig-v1, got {}",
            entry.attestation
        ));
    }
...
fn validate_remove(op: &ForgeRegistryRemove) -> Result<(), String> {
...
fn validate_contributor_add(
    state: &ForgeRegistryState,
    op: &ForgeRegistryContributor,
) -> Result<(), String> {
    if op.schema_version != 1 {
        return Err(format!("unsupported schema_version {}", op.schema_version));
    }
...
fn clear_pending_invite(state: &mut ForgeRegistryState, prefix: &str, fingerprint: &str) {
...
fn parse_state(bytes: &[u8]) -> Result<ForgeRegistryState, ContractError> {
    if bytes.is_empty() {
        return Ok(ForgeRegistryState::empty());
    }
    serde_json::from_slice(bytes).map_err(|e| ContractError::Deser(e.to_string()))
}

fn tombstone_blocks(state: &ForgeRegistryState, prefix: &str, seq: u64) -> bool {
    state
        .removed
        .get(prefix)
        .map(|t| t.seq >= seq)
...
fn merge_states(
    mut current: ForgeRegistryState,
    incoming: ForgeRegistryState,
) -> Result<ForgeRegistryState, ContractError> {
...
fn apply_envelope(
    current: &mut ForgeRegistryState,
    bytes: &[u8],
) -> Result<(), ContractError> {
...
#[serde(untagged)]
enum UpdateEnvelope {
    Upsert { upsert: ForgeRegistryEntry },
    Remove { remove: ForgeRegistryRemove },
    AddContributor {
        add_contributor: ForgeRegistryContributor,
    },
    RemoveContributor {
        remove_contributor: ForgeRegistryContributor,
    },
    AddPendingInvite {
        add_pending_invite: ForgeRegistryPendingInvite,
    },
    RemovePendingInvite {
        remove_pending_invite: ForgeRegistryPendingInvite,
    },
}
...
mod tests {
    ...
    fn dual_sig_round_trip() {
        ...
        let mut entry = ForgeRegistryEntry {
            schema_version: 1,
            repo_prefix: prefix,
            label: "demo".into(),
            name: Some("Demo".into()),
    ...
    fn remove_tombstone_blocks_stale_upsert() {
        ...
        let mut state = ForgeRegistryState::empty();
        merge_entry(&mut state, entry.clone()).unwrap();

        let mut op = ForgeRegistryRemove {
            schema_version: 1,
            repo_prefix: prefix.clone(),
