//! GitForge vault Freenet contract — passwordless schema v4.
//!
...
// const SIGN_DOMAIN: &[u8] = b"gitforge.vault.v3\0";
// const PARAMS_PREFIX: &str = "gitforge-vault-v1:";
// const SCHEMA_VERSION: u32 = 3;
// NEW CODE - TESTING: GitForge passwordless vault
const SIGN_DOMAIN: &[u8] = b"gitforge.vault.v4\0";
const PARAMS_PREFIX: &str = "gitforge-vault-v1:";
...
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq)]
pub struct ForgeVaultState {
    pub schema_version: u32,
    /// hex(blake3("gitforge-vault-v1" ‖ seed)); must match params.
    pub vault_id: String,
...
pub fn signing_payload(state: &ForgeVaultState) -> Result<Vec<u8>, String> {
    let envelopes_json =
...
fn validate_vault_state(state: &ForgeVaultState, expected_vault_id: &str) -> Result<(), String> {
...
fn ops_envelope_delta_allowed(prev: &ForgeVaultState, next: &ForgeVaultState) -> Result<(), String> {
...
fn parse_state(bytes: &[u8]) -> Result<Option<ForgeVaultState>, ContractError> {
    if bytes.is_empty() {
        return Ok(None);
    }
...
fn apply_upsert(
    current: Option<ForgeVaultState>,
    next: ForgeVaultState,
    expected_vault_id: &str,
) -> Result<ForgeVaultState, ContractError> {
...
fn apply_update_bytes(
    ...
    bytes: &[u8],
    expected_vault_id: &str,
) -> Result<Option<ForgeVaultState>, ContractError> {
    if bytes.is_empty() {
        return Ok(current);
    }
    ...
    if let Ok(s) = serde_json::from_slice::<ForgeVaultState>(bytes) {
