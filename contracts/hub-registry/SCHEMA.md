# HubRegistry contract (schema)

Public FreenetHub discovery listings.

**Bridge (Express):** `hub-registry.json` with `attestation: "local-bundle-v1"`.

**Network (WASM):** this contract — only `attestation: "dual-sig-v1"`.

## Invariants

- Open register (anyone with a FreenetHub identity delegate may list a repo they own).
...
| `identity_name` | string | **Unused for display** — always empty on new listings. Username comes from HubProfile for this fingerprint. Kept in signing payload for compatibility. |
| `identity_email` | string? | Unused for display (HubProfile `public_email`); empty on new listings |
...
`gitatlas.register.v2\0` + nul-terminated fields
...
Heavy / appendable / sealed repo data lives on **HubRepoMeta**
(`gitatlas-repo-v1:<prefix>`), not here.

## Parameters

UTF-8 `gitatlas-registry-v1` (singleton instance for a given WASM code hash).
...
- Or full `HubRegistryState` merge.
...
Unregister signing domain: `gitatlas.unregister.v1\0` + nul-terminated
...
Signing domain: `gitatlas.contributor.add.v1\0` + nul-terminated
...
Signing domain: `gitatlas.contributor.remove.v1\0` + same fields as add.
...
totals). Distinct from HubProfile inbox messages (which are only the invitee’s
...
Signing domain: `gitatlas.pending-invite.add.v1\0` + nul-terminated
`repo_prefix`, `identity_fingerprint`, `repo_owner_vk`, then `seq` u64 LE,
then nul-terminated `updated_at`.
...
Signing domain: `gitatlas.pending-invite.remove.v1\0` + same fields as add.
