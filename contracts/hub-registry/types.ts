/** Shared HubRegistry listing shape (bridge JSON + WASM contract). */
...
export interface HubRegistryEntry {
  ...
  /** Unused for display (always empty on dual-sig listings). Use HubProfile. */
...
export interface HubRegistryRemove {
  schema_version: number;
  repo_prefix: string;
  identity_fingerprint: string;
...
export interface HubRegistryContributor {
  schema_version: number;
  repo_prefix: string;
  identity_fingerprint: string;
  repo_owner_vk: string;
...
export interface HubRegistryPendingInvite {
  schema_version: number;
  repo_prefix: string;
  /** Invitee fingerprint. */
  identity_fingerprint: string;
...
export interface HubRegistryState {
  ...
  repos: Record<string, HubRegistryEntry>;
  /** Soft-delete tombstones keyed by repo_prefix */
  removed?: Record<string, HubRegistryRemove>;
  /** Accepted contributors: prefix → fingerprint → grant */
  contributors?: Record<string, Record<string, HubRegistryContributor>>;
  /** Outstanding invites: prefix → invitee fingerprint → invite */
  pending_invites?: Record<string, Record<string, HubRegistryPendingInvite>>;
}
