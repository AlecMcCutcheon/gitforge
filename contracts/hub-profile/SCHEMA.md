# GitAtlas profile contract (schema)
...
`gitatlas-profile-v1:` + `freenet:id:<base58 verifying key>`
...
| `created_at` | ISO-8601 |
| `sender_vk` | **required on append** — base58 ed25519 VK of sender GitAtlas identity |
...
`gitatlas.profile.v3\0` + nul fields:
...
`gitatlas.profile.inbox-append.v1\0` + nul fields:

recipient `identity_fingerprint` (contract params), `id`, `ciphertext_b64`,
`created_at`, `sender_vk`.

Verified against `sender_vk`. Proves the sender holds a GitAtlas identity
