//! GitAtlas profile Freenet contract — public bio/avatar/url + encrypted inbox.
//!
//! Addressed by identity fingerprint (not vault seed). Owner-signed upserts for
//! bio + inbox prune / inbox_pk. Inbox appends require the sender’s GitAtlas
...
// const SIGN_DOMAIN: &[u8] = b"gitatlas.profile.v2\0";
// const SCHEMA_VERSION: u32 = 2;
// NEW CODE - TESTING: profile v3 + public_meta bag
const SIGN_DOMAIN: &[u8] = b"gitatlas.profile.v3\0";
const INBOX_APPEND_DOMAIN: &[u8] = b"gitatlas.profile.inbox-append.v1\0";
const PARAMS_PREFIX: &str = "gitatlas-profile-v1:";
...
pub struct InboxMessage {
    ...
    /// Base58 ed25519 verifying key of the GitAtlas identity that signed this append.
...
struct UpsertEnvelope {
    upsert: HubProfileState,
}

/// Anyone with a GitAtlas identity may append a sealed message if they
...
pub fn signing_payload(state: &HubProfileState) -> Result<Vec<u8>, String> {
...
/// Append path: require GitAtlas identity proof (sender_vk + sender_sig).
...
fn validate_profile_state(state: &HubProfileState, expected_fp: &str) -> Result<(), String> {
...
fn parse_state(bytes: &[u8]) -> Result<Option<HubProfileState>, ContractError> {
    if bytes.is_empty() {
        return Ok(None);
    }
...
fn apply_upsert(
    current: Option<HubProfileState>,
    next: HubProfileState,
    expected_fp: &str,
) -> Result<HubProfileState, ContractError> {
...
fn apply_update_bytes(
    ...
    bytes: &[u8],
    expected_fp: &str,
) -> Result<Option<HubProfileState>, ContractError> {
    if bytes.is_empty() {
        return Ok(current);
    }
...
fn validate_profile_core(state: &HubProfileState, expected_fp: &str) -> Result<(), String> {
    if state.schema_version != SCHEMA_VERSION {
        return Err(format!("unsupported schema_version {}", state.schema_version));
    }
    if state.identity_fingerprint != expected_fp {
