//! ForgeRepoMeta — per-repo settings + public/private message channels.
//!
//! Addressed by `gitforge-repo-v1:{repo_prefix}`. Keeps appendable traffic
//! off the global ForgeRegistry Discover singleton.
...
const UPSERT_DOMAIN: &[u8] = b"gitforge.repo-meta.upsert.v1\0";
const APPEND_PUBLIC_DOMAIN: &[u8] = b"gitforge.repo-meta.append-public.v1\0";
const APPEND_PRIVATE_DOMAIN: &[u8] = b"gitforge.repo-meta.append-private.v1\0";
const PARAMS_PREFIX: &str = "gitforge-repo-v1:";
...
pub fn upsert_signing_payload(state: &ForgeRepoMetaState) -> Result<Vec<u8>, String> {
    let settings_json =
...
fn validate_upsert(state: &ForgeRepoMetaState, expected_prefix: &str) -> Result<(), String> {
...
fn parse_state(bytes: &[u8]) -> Result<Option<ForgeRepoMetaState>, ContractError> {
    if bytes.is_empty() {
        return Ok(None);
    }
...
fn apply_upsert(
    current: Option<ForgeRepoMetaState>,
    next: ForgeRepoMetaState,
    expected_prefix: &str,
) -> Result<ForgeRepoMetaState, ContractError> {
