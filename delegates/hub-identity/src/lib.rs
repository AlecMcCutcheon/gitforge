//! FreenetHub identity / repo-owner delegate.
//!
//! Holds ed25519 secrets in the node; signs HubRegistry dual-sig listings and
...
// const SIGN_DOMAIN: &[u8] = b"freenethub.register.v1\0";
// const UNREGISTER_DOMAIN: &[u8] = b"freenethub.unregister.v1\0";
// const VAULT_SIGN_DOMAIN: &[u8] = b"freenethub.vault.v3\0";
// const PROFILE_SIGN_DOMAIN: &[u8] = b"freenethub.profile.v1\0";
// const STAR_DOMAIN: &[u8] = b"freenethub.star.v1\0";
// const UNSTAR_DOMAIN: &[u8] = b"freenethub.unstar.v1\0";
// NEW CODE - TESTING: GitAtlas domains (clean break)
...
const INBOX_APPEND_DOMAIN: &[u8] = b"gitatlas.profile.inbox-append.v1\0";
const VAULT_SIGN_DOMAIN: &[u8] = b"gitatlas.vault.v4\0";

fn default_sig_kind_owner() -> String {
    "owner".into()
}
// OLD CODE - KEEP UNTIL CONFIRMED WORKING
// const PROFILE_SIGN_DOMAIN: &[u8] = b"gitatlas.profile.v2\0";
// NEW CODE - TESTING: profile v3 + public_meta
const PROFILE_SIGN_DOMAIN: &[u8] = b"gitatlas.profile.v3\0";
const REPO_META_UPSERT_DOMAIN: &[u8] = b"gitatlas.repo-meta.upsert.v1\0";
const REPO_META_APPEND_PUBLIC_DOMAIN: &[u8] = b"gitatlas.repo-meta.append-public.v1\0";
const REPO_META_APPEND_PRIVATE_DOMAIN: &[u8] = b"gitatlas.repo-meta.append-private.v1\0";
const STAR_DOMAIN: &[u8] = b"gitatlas.star.v1\0";
const UNSTAR_DOMAIN: &[u8] = b"gitatlas.unstar.v1\0";

struct HubIdentityDelegate;
...
enum Request {
    ...
    SignRegister {
        ...
        // NEW CODE - TESTING: About website + topics on HubRegistry
        #[serde(default)]
    ...
    /// Soft-unregister from HubRegistry Discover.
    SignUnregister {
        nonce: String,
        prefix: String,
        seq: u64,
    ...
    /// Dual-sign HubRegistry contributor grant (after ImportRepoKey / legacy).
    SignContributorAdd {
        nonce: String,
        prefix: String,
        seq: u64,
    ...
    /// Dual-sign HubRegistry contributor remove (self-leave or owner revoke).
    ...
    /// Owner: dual-sign HubRegistry pending invite (repo-level invite row).
    ...
    /// Sign a HubProfile inbox append (proves sender holds this identity).
    SignInboxAppend {
        nonce: String,
    ...
    /// Sign RepoState `pages` extension (GitAtlas Pages public metadata).
    SignRepoPages {
        nonce: String,
        prefix: String,
        /// Current RepoState bytes (hex).
    ...
    Logout,
    /// Sign GitAtlas vault public state (passwordless v4 — SPA builds ciphertext).
    SignVault {
        vault_id: String,
        username: String,
    ...
    /// Sign GitAtlas profile public state (bio / avatar / url / inbox / public_meta).
    SignProfile {
        username: String,
        #[serde(default)]
        public_email: String,
    ...
    /// Dual-sign HubRepoMeta upsert (settings + seal_pk).
    SignRepoMetaUpsert {
        nonce: String,
        prefix: String,
        #[serde(default)]
    ...
    /// Identity-sign public channel append on HubRepoMeta.
    ...
    /// Identity-sign private channel append on HubRepoMeta.
...
/// HubRegistry pending invite row (add / cancel / decline).
#[derive(Serialize, Deserialize)]
...
fn sign_register(
    ...
    // NEW CODE - TESTING: HubRegistry identity fields are fingerprint-only;
    // username/email for display come from HubProfile. Signing field stays empty.
...
fn sign_vault(
    ...
    if dek_wrap.is_empty() {
        return Response::Error {
            message: "identity_dek_wrap_json required for GitAtlas vault v4".into(),
            nonce: None,
        };
    }
...
#[delegate]
impl DelegateInterface for HubIdentityDelegate {
    fn process(
        ctx: &mut DelegateCtx,
        ...
        // None (local command WS / CLI `gitatlas-vault --import-local-delegate`).
        ...
        match &origin {
            ...
            Some(MessageOrigin::Delegate(_)) => {
                return Err(DelegateError::Other(
                    "hub-identity does not accept inter-delegate calls".into(),
                ));
            }
            other => {
                return Err(DelegateError::Other(format!(
                    "hub-identity rejects origin {other:?}"
                )));
            }
        }
