//! GitForge Pages delegate — website signing keys for GitForge Pages.
//!
//! Holds per-repo ed25519 secrets (`pages_sk:<prefix>`); compresses ustar → xz
//! and signs WebContainerMetadata (version || archive).
//! ExportKeys returns signing material for ForgeVault seal / local fdev use
...
enum Request {
    ...
    /// Export all pages signing secrets (for ForgeVault seal / fdev).
    ExportKeys {
        nonce: String,
    },
...
#[delegate]
impl DelegateInterface for ForgePagesDelegate {
    fn process(
        ctx: &mut DelegateCtx,
        ...
        match &origin {
            ...
            Some(MessageOrigin::Delegate(_)) => {
                return Err(DelegateError::Other(
                    "forge-pages does not accept inter-delegate calls".into(),
                ));
            }
            other => {
                return Err(DelegateError::Other(format!(
                    "forge-pages rejects origin {other:?}"
                )));
            }
        }
